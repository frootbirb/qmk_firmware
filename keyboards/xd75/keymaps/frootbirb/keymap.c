...
void copy_paste_all(qk_tap_dance_state_t *state, void *user_data) {
  switch (state->count) {
    case 1:
      register_code(KC_RGUI); register_code(KC_A); unregister_code(KC_A); register_code(KC_C); unregister_code(KC_C); unregister_code(KC_RGUI); 
      break;
    case 2:
      register_code(KC_RGUI); register_code(KC_V); unregister_code(KC_V); unregister_code(KC_RGUI); 
      break;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    KEYMAP(
        ...
        KC_RCMD,		KC_LALT,	KC_LCTRL,	TD(CP),	KC_SPC,	LT(NUM, KC_ENT),	KC_PAST,	KC_KP_DOT,	KC_KP_0,	KC_SPC,	MO(NAV),	xx,		KC_LEFT,	KC_DOWN,	KC_RGHT
	),
